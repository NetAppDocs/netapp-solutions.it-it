...
summary: NetApp ha ricevuto una richiesta di ricerca di file duplicati da un singolo volume o da più volumi. NetApp ha fornito la seguente soluzione. 
---
= File duplicati
:hardbreaks:
:allow-uri-read: 
...
link:xcp-bp-using-the-xcp-data-mover-to-migrate-large-files.html["Precedente: Utilizzo di XCP Data Mover per la migrazione di file di grandi dimensioni."]

[role="lead"]
NetApp ha ricevuto una richiesta di ricerca di file duplicati da un singolo volume o da più volumi. NetApp ha fornito la seguente soluzione.

Per un singolo volume, eseguire i seguenti comandi:
...
....
Per più volumi, eseguire i seguenti comandi:
...
....
link:xcp-bp-specific-date-based-scan-and-copy-of-data.html["Successivo: Scansione e copia dei dati specifiche basate sulla data."]
