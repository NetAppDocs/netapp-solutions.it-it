...
* *Requisiti del software Oracle.* In generale, un database standby fisico deve avere la stessa versione di Database Home del database primario, incluse le eccezioni del set di patch (PSE), gli aggiornamenti di patch critici (CPU) e gli aggiornamenti del set di patch (PSU), a meno che non sia in corso un processo di applicazione delle patch di standby di Oracle Data Guard (come descritto nella nota 1265700.1 del supporto Oracle all'indirizzo link:https://support.oracle.com.["support.oracle.com"^]
