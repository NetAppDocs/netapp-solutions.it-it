...
image:aws_ora_fsx_ec2_vldb_architecture.png["Questa immagine fornisce un quadro dettagliato dell'implementazione della fusione incrementale di Oracle VLDB nel cloud pubblico AWS con FSX ONTAP."]
...
. Dalla console AWS EC2, implementa i cluster ha di storage Amazon FSX ONTAP per ospitare i volumi NFS che memorizzano la copia dell'immagine di standby del database Oracle. Se non hai dimestichezza con l'implementazione dello storage FSX, consulta la documentazione link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/creating-file-systems.html["Creazione di file system FSX ONTAP"^] per istruzioni dettagliate.
...
+
image:aws_ora_fsx_ec2_vldb_vol.png["Figura che mostra la finestra di dialogo input/output o rappresenta il contenuto scritto"]
...
----
. Accedere al database Oracle tramite sqlplus, abilitare il tracciamento delle modifiche dei blocchi per un backup incrementale più veloce e modificare l'area di ripristino flash Oracle su FSX ONTAP mount se si trova attualmente sullo storage primario. Ciò consente di eseguire il backup del file di controllo predefinito RMAN/spfile e dei registri archiviati su FSX ONTAP NFS mount per il ripristino.
+
[source, cli]
----
sqlplus / as sysdba
...
[%collapsible%open]
====
In caso di guasto in caso di perdita dello storage primario e dell'host dell'istanza del DB EC2, il ripristino non può essere eseguito dal server originale. Per fortuna, hai ancora una copia dell'immagine di backup del database Oracle sul file system NFS FSX ONTAP ridondante. È possibile eseguire rapidamente il provisioning di un'altra istanza EC2 DB identica e montare facilmente la copia dell'immagine del VLDB sul nuovo host EC2 DB tramite NFS per eseguire il ripristino. In questa sezione, illustreremo le procedure passo-passo per farlo.
...
SQL>
....
. Come utente oracle, eseguire il backup incrementale RMAN e unirlo per svuotare la transazione al set di backup su FSX ONTAP mount.
+
....
[oracle@ip-172-30-15-99 ~]$ rman target / nocatalog
...
. Configurare l'ambiente oracle in modo simile all'host dell'istanza primaria di EC2 DB ora_01, ad esempio oratab, orainst.loc e oracle user .bash_profile. È buona norma eseguire il backup di questi file nel punto di montaggio NFS di FSX ONTAP.
. La copia dell'immagine di backup del database Oracle su FSX ONTAP NFS mount è memorizzata in un cluster FSX che estende le zone di disponibilità di AWS per ridondanza, elevata avilabilità e performance elevate. Il file system NFS può essere facilmente montato su un nuovo server fino a quando la rete è raggiungibile. Le seguenti procedure montano la copia dell'immagine di un backup di Oracle VLDB su un host di istanza EC2 DB appena predisposto per il ripristino.
+
...
sudo mount 172.30.15.19:/ora_01_copy /nfsfsxn -o rw,bg,hard,vers=3,proto=tcp,timeo=600,rsize=262144,wsize=262144,nointr
----
. Convalidare la copia dell'immagine di backup del database Oracle sul punto di montaggio NFS FSX ONTAP.
+
....
...
....
. Verificare i log Oracle archiviati disponibili sul supporto NFS di FSX ONTAP per il ripristino e annotare l'ultimo numero di sequenzialità del log. In questo caso, è 175. Il nostro punto di ripristino è fino al numero di sequenza di registrazione 176.
+
....
...
. In qualità di utente oracle, creare un file init Oracle generico nella directory €ORACLE_HOME/dbs con le directory amministrative corrette configurate. Soprattutto, hanno il percorso di montaggio NFS ONTAP point to FSX di Oracle `flash recovery area` come definito nell'istanza primaria di Oracle VLDB.  `flash recovery area` la configurazione è illustrata nella sezione `Setup Oracle RMAN incremental merge to image copy on FSx`. Impostare il file di controllo Oracle sul file system NFS FSX ONTAP.
+
[source, cli]
----
...
Il ripristino del database Oracle VLDB viene completato dalla copia dell'immagine di backup sul file system NFS FSX ONTAP a un nuovo host di istanza EC2 DB.
