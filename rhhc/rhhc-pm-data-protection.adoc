...
NOTE: Se i pod delle applicazioni non sono in esecuzione, potrebbe essersi verificato un errore dovuto ai vincoli del contesto di protezione.
image:rhhc-scc-error.png[""]
Correggere l'errore modificando `runAsUser` e. `fsGroup` campi in `statefuleset.apps/postgresql` oggetto con l'uid che si trova nell'output di `oc get project` comando come illustrato di seguito.
