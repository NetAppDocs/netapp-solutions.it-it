...
Il cliente del caso di utilizzo ibrido può utilizzare Cloud Sync per migrare i dati on-premise dai dati NFS, CIFS e S3 al cloud e viceversa per l'elaborazione ai utilizzando GPU come quelle di un cluster NVIDIA. Sia Cloud Sync che lo strumento di migrazione XCP vengono utilizzati per la migrazione dei dati NFS su NetApp ONTAP NFS.
