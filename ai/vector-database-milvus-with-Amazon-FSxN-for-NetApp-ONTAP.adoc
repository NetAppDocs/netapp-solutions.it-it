...
. Creare una cartella di montaggio e montare su di essa Amazon FSX ONTAP per NetApp ONTAP.
+
....
...
ubuntu@ip-172-31-29-98:~$
----
. Per convalidare la funzionalità di lettura e scrittura del database vettoriale e dei relativi dati in Amazon FSX ONTAP per NetApp ONTAP, abbiamo utilizzato l'SDK Python Milvus e un programma di esempio di PyMilvus. Installare i pacchetti necessari usando 'apt-get install python3-numpy python3-pip' e installare PyMilvus usando 'pip3 install pymilvus'.
. Convalidare le operazioni di scrittura e lettura dei dati da Amazon FSX ONTAP per NetApp ONTAP nel database vettoriale.
+
[source, python]
----
...
----
+
Questa sezione dimostra in modo efficace come i clienti possono implementare e utilizzare una configurazione Milvus standalone all'interno dei container Docker, utilizzando Amazon NetApp FSX ONTAP per lo storage dei dati NetApp ONTAP. Il setup consente ai clienti di sfruttare la potenza dei database vettoriali per la gestione dei dati ad alta dimensione e l'esecuzione di query complesse, il tutto all'interno dell'ambiente scalabile ed efficiente dei container Docker. Creando un'istanza di Amazon FSX ONTAP per NetApp ONTAP e una corrispondente istanza di EC2, i clienti possono garantire un utilizzo ottimale delle risorse e una gestione dei dati. La convalida riuscita delle operazioni di scrittura e lettura dei dati da FSX ONTAP nel database vettoriale fornisce ai clienti la garanzia di operazioni di dati affidabili e coerenti. Inoltre, la possibilità di elencare (leggere) i dati dai workload ai tramite il protocollo S3 offre una migliore accessibilità dei dati. Questo processo completo, pertanto, offre ai clienti una soluzione solida ed efficiente per gestire le loro operazioni sui dati su larga scala, sfruttando le funzionalità di Amazon FSX ONTAP per NetApp ONTAP.
