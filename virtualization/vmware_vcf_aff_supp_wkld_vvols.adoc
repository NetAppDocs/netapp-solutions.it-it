...
* Creare adattatori vmkernel per NFS sugli host ESXi per il dominio del carico di lavoro VI.
...
NetApp consiglia progettazioni di rete ridondanti per NFS. Il diagramma seguente illustra un esempio di configurazione ridondante, che fornisce tolleranza agli errori per sistemi di archiviazione, switch, schede di rete e sistemi host. Consultare il NetApp link:https://docs.netapp.com/us-en/ontap/san-config/index.html["Riferimento alla configurazione SAN"] per ulteriori informazioni.
...
.Creare gruppi di porte distribuite per il traffico NFS
[%collapsible]
====
