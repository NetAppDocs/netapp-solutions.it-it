...
[%collapsible%open]
====
Lo scopo di questa soluzione è dimostrare la protezione dei dati delle macchine virtuali in esecuzione in VMware vSphere, domini di carico di lavoro VCF VI o domini di gestione VCF. Le macchine virtuali presenti in questa soluzione sono ospitate su datastore NFS forniti da NetApp ONTAP. Questa soluzione presuppone che i seguenti componenti siano configurati e pronti per l'uso:
